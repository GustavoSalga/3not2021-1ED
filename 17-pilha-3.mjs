...
for(let i = 0; i < expr.length; i++) {
    switch(expr.charAt(i)) { // os casos { [ e ( estão sendo adicionados na pilha
        case '{': // Buscando o caso de chaves que abrem. Viradas para lá > {
            analisador.push({tipo: 'CH', pos: i}) // Defino o tipo do item encontrado (CH de Chave) e qual posição dele pos: i
        ...
        case '(':
            ...
            break
        case '}':
            info = analisador.pop() // os casos ] ) } estão sendo retirados da pilha
            if(info && info.tipo == 'CH') {      // Aqui ele compara, por ordem de retirada da pilha se o TIPO retirado é o mesmo do inserido por último na pilha
                console.log(`Chave aberta na posição ${info.pos} e fechada na posição ${i}`)
            }
            else {
                console.log(`ERRO: chave fechando na posição ${i} não tem a abertura correspondente`) // Se não for, a expressão está errada
            }
            break
        case ']':
            info = analisador.pop()
